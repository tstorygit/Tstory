...
async function handleEnrichVocab() {
    ...
    try {
        ...
        const sys = `You are a Japanese NLP tool. Return a JSON object mapping each exact input string to its dictionary base form (infinitive). Example: ["食べた", "猫"] -> {"食べた": "食べる", "猫": "猫"}. Return ONLY valid JSON without markdown formatting.`;
...
function showEnrichReviewModal(updates) {
    ...
    box.style.cssText = 'background:var(--surface-color);width:100%;max-width:500px;max-height:85vh;border-radius:12px;display:flex;flex-direction:column;box-shadow:0 10px 30px rgba(0,0,0,0.5);overflow:hidden;';
    ...
    header.innerHTML = `<h3 style="margin:0;font-size:18px;color:var(--text-main);">Review Enriched Base Forms</h3>`;
    ...
    updates.forEach((u, i) => {
        ...
        row.innerHTML = `
            ...
            <input type="text" id="enrich-input-${i}" value="${u.base}" style="flex:1;padding:8px;border:1px solid var(--border-color);border-radius:6px;background:var(--surface-color);color:var(--text-main);font-size:14px;">
        `;
    ...
    saveBtn.textContent = 'Save Base Forms';
    saveBtn.className = 'primary-btn';
    saveBtn.style.flex = '2';
    saveBtn.onclick = () => {
        const finalUpdates = updates.map((u, i) => ({
            key: u.key,
            base: document.getElementById(`enrich-input-${i}`).value.trim() || u.key
        }));
