...
// ✅ FIX: Import ALL engine functions statically — dynamic import().then() returns Promises,
//         which caused "getActiveWeapons(...).map is not a function"
import { initCanvas, startRun, stop, applyUpgrade, applyPenalty, resume,
         getActiveWeapons, getActivePassives, getElapsedTime } from './surv_engine.js';
...
export function init(screens, onExit) {
    ...
    // ✅ FIX: Pass direct function references — they return values synchronously
        ...
        return `
            <div class="surv-shrine-item">
                <div class="surv-shrine-info">
                    <div class="surv-shrine-name">${u.icon} ${u.name} <span class="surv-shrine-rank">Lv.${lvl}/${max}</span></div>
